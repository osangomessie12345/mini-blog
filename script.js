...
async function handleSubmit() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      message.textContent = data.message || (mode === 'login' ? 'Connexion réussie !' : 'Inscription réussie !');
      
      setTimeout(() => {
        window.location.href = 'page-communaute.html'; // Redirection pour les deux cas
      }, 1000);
    } else {
      message.style.color = 'var(--error-color)';
